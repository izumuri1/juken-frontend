...
CREATE OR REPLACE FUNCTION "public"."get_workspace_members_safe"("workspace_id_param" "uuid") RETURNS TABLE("id" "uuid", "user_id" "uuid", "role" "text", "joined_at" timestamp without time zone, "username" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
  -- アクセス権限チェック: ワークスペースのオーナーまたはメンバーのみアクセス可能
  IF NOT EXISTS (
...
CREATE POLICY "Anyone can read active tokens" ON "public"."invitation_tokens" FOR SELECT USING ((("expires_at" > "now"()) AND ("current_uses" < "max_uses")));
...
CREATE POLICY "Authenticated users can insert workspace members" ON "public"."workspace_members" FOR INSERT WITH CHECK (("auth"."uid"() IS NOT NULL));



CREATE POLICY "Members can view their workspace memberships" ON "public"."workspace_members" FOR SELECT TO "authenticated" USING (("user_id" = "auth"."uid"()));



CREATE POLICY "Token creators can update" ON "public"."invitation_tokens" FOR UPDATE USING (("created_by" = "auth"."uid"()));



CREATE POLICY "Users can join workspace via invitation" ON "public"."workspace_members" FOR INSERT TO "authenticated" WITH CHECK (("user_id" = "auth"."uid"()));



CREATE POLICY "Users can read other users basic info" ON "public"."users" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "Users can view own membership and owned workspaces" ON "public"."workspace_members" FOR SELECT USING ((("user_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
...
CREATE POLICY "Workspace owners can create tokens" ON "public"."invitation_tokens" FOR INSERT WITH CHECK (("workspace_id" IN ( SELECT "workspaces"."id"
   FROM "public"."workspaces"
...
CREATE POLICY "workspace_insert" ON "public"."workspaces" FOR INSERT TO "authenticated" WITH CHECK (("owner_id" = "auth"."uid"()));
...
CREATE POLICY "workspace_select" ON "public"."workspaces" FOR SELECT TO "authenticated" USING ((("owner_id" = "auth"."uid"()) OR ("id" IN ( SELECT "workspace_members"."workspace_id"
   FROM "public"."workspace_members"
  WHERE ("workspace_members"."user_id" = "auth"."uid"())))));
...
GRANT ALL ON FUNCTION "public"."get_workspace_members_safe"("workspace_id_param" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_workspace_members_safe"("workspace_id_param" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_workspace_members_safe"("workspace_id_param" "uuid") TO "service_role";
...
GRANT SELECT ON TABLE "public"."invitation_tokens" TO "anon";
GRANT SELECT,INSERT,UPDATE ON TABLE "public"."invitation_tokens" TO "authenticated";
